...
class Model
{
	...
	public $apiKey = "9192df2611317fe0e07aaeeda815e552a803b26a";
	...
	public function __construct($options = array())
	{
		...
		$this->data["object"] = $this->tableName;

		$this->pdo = Database::connect();

		// HTTP worker set up
		$this->http = new HTTP(new Resty(), array("Accept" => "application/json", "X_API_VERSION" => 0, "Authorization" => $this->apiKey));
		...
		// Get options from the options table and include them as array
		$options = $this->pdo->query("SELECT * FROM options")->fetchAll(PDO::FETCH_ASSOC);
		$site_options = array();
		foreach ($options as $o) { $site_options[$o["key"]] = $o["value"]; }
		$this->data["site_options"] = $site_options;
	}
	 ...
	 * @param  string $ignore If true, use the IGNORE keyword
	 */
	public function create(array $values, $ignore = false)
	{
		...
		$keyword = !$ignore ? "INSERT" : "INSERT IGNORE";

		$sql = "{$keyword} INTO {$this->tableName} ({$columns}) VALUES ({$qMarks})";
	 ...
	 * @param  string $sql    Unprepared SQL statement with `?` representing values
	 * @param  array  $values Array of values to populate `?` with
